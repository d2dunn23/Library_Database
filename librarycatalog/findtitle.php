...
<head>
	...
	<?php
		...
		$search=mysqli_real_escape_string($con, $_POST['stitle']);
		
		//search query
		$sel_query="Select * FROM books, writtenby, authors WHERE books.isbn=writtenby.isbn AND writtenby.authorid=authors.authorid AND books.title LIKE '%{$search}%' ORDER BY books.title";
